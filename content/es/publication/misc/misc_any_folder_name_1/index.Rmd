...
![](https://lh3.googleusercontent.com/pw/ACtC-3fdw0DERIdjiQAwUmysAv2ZoJugqtDcJ31BQTAZIk5HAkrQSK2cwskYKFFkjFiDFScvGMa3HIO21Z-qZo9oqotLg7CTJWG3LTgmnyJvSQWk4Z1tOWqqC6RbO4_M7yCUY5fVEvdUzrx5Q5JVchdNNJrt=w678-h557-no?authuser=3){#fig:Waterfield width="100%"}
...
rootstocks such as *Cucurbita maxima* × *Cucurbita moschata* and
...
[@Davis2008; @Hassell2008; @Thies2010; -@Thies2015;
...
[@Huitron2007a; @Thies2010; -@Thies2015; -@Thies2016]. In
addition, watermelon grafted onto *C. amarus* yielded more than those
grafted onto *L. siceraria*, *C. maxima* × *C. moschata* or
*Praecitrullus fistulosus*, without affecting the quality and the size
of the fruits [@Kyriacou2016; @Thies2015; @Fredes2017a]. However,
...
the egg suspension was filtered through a 74 μm sieve, and then, the
eggs were collected on a 25 μm sieve and placed on Baermann trays
...
::: {#tab:table201}
  ...
  *M. javanica*        MJ05        Barcelona           Avirulent      @Ornat2007
  ...
  : Table 1. *Meloidogyne* species and isolates used in pot experiments,
...
containing sterile sand and maintained in a growth chamber at 25 ±
...
[@HusseyR.S.Barker1973c], passed through a 74 μm aperture screen
and collected in a 25 μm sieve for final counting. Reproduction
...
Baermann trays [@Whitehead1965a]) and incubated at 27 ± 2 C for one
week. Afterwards, the J2 were collected using a 25 μm aperture
...
CI64, CI67, RO, and the commercial hybrid *C. maxima* × *C.
...
than the weight of those produced by the ungrafted plants (5.5 ± 0.2
*vs.* 4.7 ± 0.5 kg) but with a significant decrease (*P* $<$ 0.05)
of soluble solids (9.45 ± 0.27 *vs.* 10.67 ± 0.32 Bx). The
...
Data are the mean ± standard error of 10 replicates in experiments 1
...
^a^Reproduction index = 100 × (number of eggs per plant produced
in the CI accessions or RO / mean number of eggs per plant produced in
the susceptible cv. Sugar Baby).

::: {#tab:table203}
  Plant host   Galling index^a^   Eggs per plant (×10$^{2}$)   Reproduction index (%)^b^
  ------------ ------------------ ----------------------------------- ---------------------------
  Sugar Baby   5.0 ± 0.6 a    1031 ± 484 a                    
  Robusta      2.8 ± 0.4 b    51 ± 11 b                       4.4 ± 0.9
  BGV0005164   2.5 ± 0.5 b    16 ± 10 b                       1.4 ± 0.9
  BGV0005167   1.5 ± 0.5 b    15 ± 12 b                       1.3 ± 1.0
...
Data are the mean ± standard error of 10 replicates. Different
...
^b^Reproduction index = 100 × (number of eggs per plant produced
in the CI accessions or Robusta / mean number of eggs per plant produced
in the susceptible cv. Sugar Baby).

^a^Data are the mean ± standard error of 10 replicates. Data in the
...
siceraria*, and *C. maxima* × *C. moschata* [@Thies2003],
...
*Cucurbita maxima* × *Cucurbita moschata* and *Lagenaria
...
firstly filtered through a 74 μm sieve and finally collected on a 25
μm sieve and placed on Baermann trays [@Whitehead1965a]. The J2
...
[@Whitehead1965a]) and incubated at 27 ± 2 C for one week. J2 were
collected using a 25 μm aperture screen, counted, and expressed as
...
incubated in Baermann trays at 27 ± 2 C to allow J2 emergence to
...
were collected daily for 10 days using a 25 μm sieve and stored at 9
...
maintained in a growth chamber at 25 ± 2 C with a 16:8 h
...
Data are mean ± standard error of 7 replicates. Values followed by
...
index = 100 × (number of eggs per plant in a given rootstock /
...
lycopersicum* × *S. habrochaites*) [@Daunay2008]. However, the
...
interspecific hybrids of *S. integrifolium* × *S. melongena*,and
...
vermiculite-filled trays and incubated in a growth chamber at 25 ± 2
...
(J2)/cm^3^ of sand and maintained in a growth chamber at 25 ± 2
...
25 ± 2 C with a 16:8h (light:dark) photoperiod for 55 days. In the
...
incubated at 27 ± 2 C for a week. J2 were counted and expressed as
...
::: {#tab:table403}
  *Meloidogyne* spp.   Isolate   Crop     Cultivar   Eggs per plant ($\times 10^2$)   RI (%)$^{a}$
  -------------------- --------- -------- ---------- -------------------------------- -----------------
  *M. javanica*        MJLg      Tomato   Durinta    477.4 ± 93.8                 
                                          Monika     469.3 ± 51.5                 98.3 ± 10.8
  *M. incognita*       MILgN     Tomato   Durinta    387.9 ± 50.1\*               
                                          Monika     10.2 ± 2.5                   2.6 ± 0.6
                                 Pepper   Compas     63.9 ± 11.0                  
                                          Solfeo     57.0 ± 15.5                  89.3 ± 24.2
...
Data are mean ± standard error of 14 replicates. Values per each
...
× (number of eggs per plant in the resistant cultivar / mean
number of eggs per plant on the susceptible cultivar).

Data are mean ± standard error of 14 replicates. Data within the
...
Dunn test. ^a^Reproduction index = 100 × (number of eggs per
plant in the *S. torvum* cultivar / number of eggs per plant on the
eggplant cv. Cristal).

Data are mean ± standard error of 10 and 15 replicates in the
...
^b^Reproduction index = 100 × (number of eggs per plant in a
...
as *S. lycopersicum* × *S. habrochaites* [@Daunay2008].
...
interspecific hybrid of *S. integrifolium* × *S. melongena*, and
...
from 500 cm^3^ of soil and incubated at 27 ± 2 C for one week,
using Baermann trays [@Whitehead1965a]. J2 were collected using a 25
μm aperture screen, counted, and expressed as J2/100 cm^3^ of
...
at 27 ± 2 C to allow J2 emergence to determine the putative
...
μm sieve, and stored at 9 C until inoculation. Seeds of the *S.
...
incubated in a growth chamber at 25 ± 2 C and 16:8h (light:dark)
...
chamber at 25 ± 2 C with a 16:8 h (light:dark) photoperiod for a
...
Data on nematode population densities in soil are the mean ±
standard error of 10 replicates. Data on GI, eggs/plant, RI and yield
are the mean ± standard error of 40 replicates. Data followed by \*
...
(Zeck, 1971). ^b^Reproduction index) = 100 × (number of eggs per
...
Data are mean ± standard error of 10 replicates. Values followed by
\* in the same column show significant differences (*P* $<$ 0.05)
...
index = 100 × (number of eggs per plant in the *S. torvum*
